jQuery(document).ready(function($){
	...
	//Change thank you page header text

	    $('.woocommerce-page.woocommerce-order-received #page-header-wrap #page-header-bg .container .row .span_6 .inner-wrap h1').text(function(i, oldText) {
	        return oldText === 'Confirmation Form' ? 'Confirmation Page' : oldText;
	    });
    ...
    // Move Name Textbox to option area, sync text typing
    var appendarea = $('div#picker_pa_name-inside-jacket, #picker_pa_initials-type').parent();
    var inputclone = $('input#wcj_product_input_fields_local_2').clone();
    var labelclone = $('input#wcj_product_input_fields_local_2').prev().clone();
    // $(inputclone).appendTo(appendarea);

    $("<div class='wcba-cloned-input'></div>").appendTo(appendarea).append(labelclone).append(inputclone);

    // $(inputclone+labelclone)

    $('input#wcj_product_input_fields_local_2').on('input', function() {
        var val = $(this).val();
        $('input#wcj_product_input_fields_local_2:not(.cloned)').val(val);
    });

    // Name inside of jacket textbox, hide on default, display on click option
    $('label[for="wcj_product_input_fields_local_2"]').parent().addClass('inactive');
    $(document).on("click", "div#picker_pa_name-inside-jacket, #picker_pa_initials-type", function(){
        if( $('.select-option[data-value="name-inside-jacket"].selected, #picker_pa_initials-type .selected').length ){
            $('label[for="wcj_product_input_fields_local_2"]').parent().removeClass('inactive');
        }
        else{
            $('label[for="wcj_product_input_fields_local_2"]').parent().addClass('inactive');
       }
    ...
    // Initial Setup
    $('.questionaire.inactive').removeClass('inactive');
    ...
    $(document).on("click", ".variations img", function(){
        ...
        if( $('img.attachment-shop_single.size-shop_single.wp-post-image').length ) {
            ...
            $('<img class="wcba-product-main-image" src="'+imgsrc+'" />').appendTo('.woocommerce-product-gallery__image a');
            $('img.zoomImg').attr('src',imgsrc);
            $('.pswp, a.woocommerce-product-gallery__trigger').hide();
        }
        else {
    ...
    if ($("body").hasClass("single-product")) {
        if($( ".wcba-previous-cart-item-variation input" ).length){
            ...
            var text = $('.wcba-item-cart-text p').html();

            $('<div class="wcba-variation-popup"><div><p>'+text+'</p><div><button>Yes</button><button>No</button></div></div><div class="window"></div></div></div>').insertBefore('body > *:nth-child(1)');
        }
    ...
    // });
		...
		$(document).bind('click keyup', '.wcba-window.active', function(e){
			if (e.keyCode == 27) {
				 $('.wcba-fabric-zoom, .wcba-window').removeClass('active');
			}
    });
    ...
    if (window.location.href.indexOf("/checkout/") > -1) {
        ...
        $('<p>Mailing Address <abbr class="required" title="required">*</abbr></p><p class="mailing-subtitle">Please provide mailing address so we can accurately quote mailing charges</p>').insertAfter('p#billing_address_1_field label');
        ...
        $(document).on("click", ".questionaire .default, .questionaire .no-measurement, .questionaire .use-existing", function(){
            // $('.woocommerce-checkout div#header-outer').addClass('position-fixed');
        });
        ...
        $(document).on("click", ".questionaire .use-existing", function(){
            $( ".questionaire" ).addClass( 'inactive' );
        ...
        $('input:radio[name="input_1"]').change(
            function(){
                if (this.checked && this.value == 'No') {
                    ...
                    $( ".questionaire > div.not-existing-customer" ).addClass( 'active' );
                    $( ".questionaire > div.existing-customer" ).removeClass( 'active' );
                    //$( ".questionaire > div:nth-child(4)" ).removeClass( 'active' );
                    $('input:radio[name="input_2"], input:radio[name="input_3"]').prop('checked', false);
                    $( ".questionaire button" ).removeClass(  );
                    $( ".questionaire button" ).addClass( 'default' );
                }
                else {
                    window.existing = true;
                    $( ".questionaire > div.existing-customer" ).addClass( 'active' );
                    $( ".questionaire > div.not-existing-customer" ).removeClass( 'active' );
                    $( ".questionaire button" ).removeClass(  );
                    $( ".questionaire button" ).addClass( 'default' );
                }
            });
        $('input:radio[name="input_2"]').change(
            function(){
                if (this.checked && this.value == 'No') {
                    //$( ".questionaire > div.not-existing-customer" ).addClass( 'active' );
                    //$( ".questionaire > div.existing-next-steps" ).removeClass( 'active' );
                    $( ".questionaire button" ).removeClass(  );
                    ...
                    $( ".questionaire button" ).removeClass(  );
                    $( ".questionaire button" ).addClass( 'use-existing' );
                }
            });
		...
		//get value if its an existing customer
		$(document).ready(function () {
    $('input:radio[name="input_1"]').on('change', function () {
         var existingCus = $('input:radio[name="input_1"]:checked').val();
				 //var newCus = $('input:radio[name="input_1"]:checked').val() == "No";
				 if ($('input:radio[name="input_1"]:checked').val() == "Yes") {
            $('#billing_existingcustomer').val(existingCus);
        } else if ($('input:radio[name="input_1"]:checked').val() == "No") {
            $('#billing_existingcustomer').val(existingCus);
            $('#billing_use_same_measurements').val('New Measurements');
        }
         // if (existingCus) {
					// 		 $('#billing_existingcustomer').val(existingCus);
         //   } else if (newCus){
		 ...
		 $('input:radio[name="input_2"]').on('change', function () {
          var sameMeasurements = $('input:radio[name="input_2"]:checked').val();
          if (sameMeasurements) {
 							 $('#billing_use_same_measurements').val(sameMeasurements);
            }
      });
});
